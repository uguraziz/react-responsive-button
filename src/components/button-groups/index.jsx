...
export default function ButtonGroups({ buttons, autoClose = false }) {
  const isMobile = useMedia(750);
  const [open, setOpen] = useState(false);

  const prapareButtons = buttons.map((button, index) => {
    ...
    return (
      <>
        ...
        <BottomSheet onDismiss={() => setOpen(false)} open={open}>
          <nav className="grid p-4 gap-y-2">{prapareButtons}</nav>
        </BottomSheet>
      </>
    );
  }

  return <nav className="flex gap-2.5">{prapareButtons}</nav>;
}
